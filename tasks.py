...
def bulk_complete_tasks():
    ...
    try:
        ...
        return jsonify({'success': False, 'message': 'タスクIDが不正です'}), 400
    ...
    tasks_map = {
        task.id: task for task in Task.query.filter(
    ...
    if not tasks_map:
        return jsonify({'success': False, 'message': '対象のタスクが見つかりません'}), 404
...
def bulk_move_tasks():
    ...
    try:
        ...
        for task_id in task_ids:
            task = tasks_map.get(task_id)
            if not task or task.category == target_category:
                continue
...
@tasks.route('/tasks/<int:task_id>/copy', methods=['POST'])
@login_required
def copy_task(task_id):
    ...
    if task.user_id != current_user.id:
        return jsonify({'success': False, 'message': 'このタスクをコピーする権限がありません'}), 403
